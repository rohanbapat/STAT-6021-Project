...
# Merging inflation factor
master_df_final <- merge(master_df_final,read.csv("inflation_factor.csv", header = T) , by = "year")

#Calculating inflation adjusted prices
master_df_final$adj_USPrice <- master_df_final$USPrice*master_df_final$factor

#Color Map
master_df_final <- merge(master_df_final,read.csv("color_map.csv", header = T)[,c(1,5)] , by = "color_id")


one_hot_part_cat <- data.frame(predict(onehot(master_df_final[, c("year", "partcat_name") ], max_levels = 57),
                            master_df_final[, c("year", "partcat_name") ]))

one_hot_color_map <- data.frame(predict(onehot(master_df_final[, c("year", "color_map") ], max_levels = 16),
                                       master_df_final[, c("year", "color_map") ]))
one_hot_color_map_ordered <- one_hot_color_map[order(one_hot_color_map$year),]
